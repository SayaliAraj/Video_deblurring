...
try:
    ...
    deform_attn_ext = load(
        ...
        sources=[
            ...
            os.path.join(module_path, 'deform_attn_cuda_pt110.cpp' if Version(torch.__version__) >= Version('1.10.0') else 'deform_attn_cuda_pt109.cpp'),
            os.path.join(module_path, 'deform_attn_cuda_kernel.cu'),
        ],
    )
except Exception as e:
    print("CUDA extension not available, running on CPU. Exception:", e)
...
class DeformAttnFunction(Function):
    ...
    def forward(ctx,
                ...
                deformable_groups=1,
                clip_size=1):
        if deform_attn_ext is None:
            raise NotImplementedError("Deformable attention is not implemented for CPU.")
        ...
        # Forward pass through the deformable attention extension (only if GPU is available)
    ...
    def backward(ctx, grad_output):
        if deform_attn_ext is None or not grad_output.is_cuda:
            raise NotImplementedError("Backward pass not implemented for CPU.")
        ...
        # Backward pass through the deformable attention extension (GPU-only)
